...
import pandas as pd  # 新增用于保存Excel
import os
...
class MaterialCNN(nn.Module):
    ...
    def __init__(self, out_dim):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=(3, 2), padding=(1, 0)),
            nn.ReLU(),
...
class ThicknessPredictor(nn.Module):
    def __init__(self, max_len, d_cnn, d_model, nhead, num_layers):
        super().__init__()
        self.max_len = max_len
...
class MultilayerNet(nn.Module):
    def __init__(self, max_len=15, d_cnn=D_CNN, d_model=D_MODEL, nhead=NHEAD, num_layers=NUM_LAYERS,
                 FILL_VALUE=FILL_VALUE):
        super().__init__()
        self.thickness_net = ThicknessPredictor(max_len, d_cnn, d_model, nhead, num_layers)
        self.fitness_net = FitnessPredictor(max_len, d_cnn, d_model, nhead, num_layers)
    ...
    def forward(self, start, end, valid_layers, mat_features, material_indices):
        ...
        thickness_mask = (material_indices != self.FILL_VALUE).float()  # [batch, seq_len]
...
def main():
    material_tensor = torch.load('material_tensor/material_tensor.pt')  # [num_materials, 1501, 2]

    # -------------------- 数据加载 --------------------
    data = torch.load('train_data103200.pt')
    ...
    model = MultilayerNet(max_layers_plus1, D_CNN, D_MODEL, NHEAD, NUM_LAYERS, FILL_VALUE).to(device)
    print(f"Total parameters: {sum(p.numel() for p in model.parameters()):,}")

    # -------------------- 损失函数和优化器 --------------------
    criterion_mse = nn.MSELoss(reduction='none')  # 使用 none 以便逐元素操作
    ...
    records = []  # 用于保存每个epoch的日志

    for epoch in range(1, EPOCHS + 1):
        model.train()
        train_loss = 0.0
        train_thickness_loss = 0.0
        ...
        for batch in train_loader:
            ...
            valid_layers = batch['valid_layers'].to(device)
            mat_features = batch['mat_features'].to(device)
            material_indices = batch['material_indices'].to(device)
            target_thickness = batch['thickness'].to(device)
            target_fitness = batch['fitness'].to(device)

            optimizer.zero_grad()
            pred_thickness, pred_fitness = model(start, end, valid_layers, mat_features, material_indices)

            # 厚度掩码（有效位置为1，填充位置为0）
            thickness_mask = (material_indices != FILL_VALUE).float()

            # 计算厚度损失：仅对有效位置计算MSE
            diff = (pred_thickness - target_thickness) * thickness_mask
            loss_thickness = (diff ** 2).sum() / (thickness_mask.sum() + 1e-8)

            # 适应度损失
            loss_fitness = criterion_mse(pred_fitness, target_fitness).mean()

            loss = LAMBDA_THICKNESS * loss_thickness + LAMBDA_FITNESS * loss_fitness
            ...
            train_thickness_loss += loss_thickness.item() * batch_size
            train_fitness_loss += loss_fitness.item() * batch_size
            all_pred_fitness.append(pred_fitness)
            all_true_fitness.append(target_fitness)

        # 计算训练指标
        train_loss /= len(train_dataset)
        train_thickness_loss /= len(train_dataset)
        train_fitness_loss /= len(train_dataset)
        all_pred_fitness = torch.cat(all_pred_fitness)
        all_true_fitness = torch.cat(all_true_fitness)
        train_mae, train_mse, train_r2 = compute_metrics(all_true_fitness, all_pred_fitness)
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                val_thickness_loss += loss_thickness.item() * batch_size
                val_fitness_loss += loss_fitness.item() * batch_size
                val_pred_fitness.append(pred_fitness)
                val_true_fitness.append(target_fitness)

        val_loss /= len(val_dataset)
        val_thickness_loss /= len(val_dataset)
        val_fitness_loss /= len(val_dataset)
        val_pred_fitness = torch.cat(val_pred_fitness)
        val_true_fitness = torch.cat(val_true_fitness)
        val_mae, val_mse, val_r2 = compute_metrics(val_true_fitness, val_pred_fitness)
        ...
        records.append({
            ...
            'train_thickness_loss': train_thickness_loss,
            'train_fitness_loss': train_fitness_loss,
            'train_mae': train_mae,
            'train_mse': train_mse,
            'train_r2': train_r2,
            'val_loss': val_loss,
            'val_thickness_loss': val_thickness_loss,
        ...
        print(f"    Train Loss: {train_loss:.6f} (Thick: {train_thickness_loss:.6f}, Fit: {train_fitness_loss:.6f})")
        print(f"    Train Fit MAE: {train_mae:.6f} MSE: {train_mse:.6f} R2: {train_r2:.6f}")
        print(f"    Val Loss: {val_loss:.6f} (Thick: {val_thickness_loss:.6f}, Fit: {val_fitness_loss:.6f})")
        print(f"    Val Fit MAE: {val_mae:.6f} MSE: {val_mse:.6f} R2: {val_r2:.6f}")
        torch.save(model.state_dict(), 'final_model.pth')

    print("Training finished. Best validation loss: {:.6f}".format(best_val_loss))
    ...
    if os.path.exists(model_path):
        ...
        print(f"No pre-trained model found at {model_path}. Please train first.")
        # 如果不想训练，可以在这里退出
        # exit()
    ...
    # 提取最后100个样本作为验证集
    val_data_tensor = data[100:100 + VALIDATION_SIZE:].clone()  # shape: [100, total_features]
    ...
    # sorted_val_data = val_data_tensor
    # sorted_true_fitness = true_fitness.cpu().numpy()
    ...
    print(f"MSE (overall): {mse_original:.6f}")
